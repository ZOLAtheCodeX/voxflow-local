...
struct ComputerActionSettingsSection: View {
    ...
    var body: some View {
        Section("Voice action controls") {
            ...
            Text("Custom prompts use your selected skill profile below. Built-in computer actions run when you say a complete phrase beginning with ‘Voxflow’. Off keeps quick capture as dictation.")
                ...
                ForEach(settings.catalog.actions) { action in
                    Toggle(isOn: Binding(get: { settings.enabledIDs.contains(action.id) },
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(action.example).font(VF.captionFont).foregroundStyle(.secondary)
                        }
                    }
                }
