...
| `VOXFLOW_OLLAMA_THINK` | `1` re-enables Ollama hidden reasoning on chat requests. Off by default: Gemma 4 thinks unless told not to, and that took warm polish from ~0.26 s to ~6 s (2026-08-24) | unset (off) |
...
- Latency receipts: `CapturePipelineTraceBuilder` measures every stage, and `InsertTimingContext` (pipeline origin + stt ms; cleanup ms measured in `DictationWorkflowCoordinator`) rides `DictationWorkflowRequest` → `TextInsertionCoordinating.insertText(_:statusSuffix:targetApp:timing:)` → the `insertions.jsonl` receipt (`stt_ms` / `cleanup_ms` / `insert_ms` / `total_ms` / `insert_method`). Rules-only polish (`activePolishProvider == "rules"`) is served in-app with provenance `rules · in-app`; the backend is only consulted for real LLM chains.
- Cockpit smart actions are single-flight: `AppState.smartActionInFlight` / `smartActionStartedAt` are set by `CockpitCoordinator.applyAction` (chips disable, top bar shows elapsed); a second dispatch returns the soft error `action_in_flight`. `SmartActionResult.degradedReason` carries the backend's floor reason so `smartActionErrorMessage` can say memory pressure / wedged runner instead of "configure a provider".
...
- SIGTERM a process by port or PID file without confirming identity — `terminateForeignListenerAsync` and `killStaleBackend` must gate on `BackendProcessManager.isVoxFlowBackendCommand(...)` (via the `BackendProcessRunning.commandLine` seam: managed `backend/app/server.py` or dev `uvicorn server:app`). Unknown/reused PIDs are refused + logged, never killed. The PID file also records the spawning session's instance stamp (`pid\nstamp`); `killStaleBackend` additionally refuses when a live `/v1/health` stamp contradicts the recorded one (probe failure falls back to the cmdline gate — the stamp check only ever reduces kills). Stamp-probe defaults are XCTest-inert like `defaultRunner()`.
- Lower cleanup consent `max_uses` below 2 (it is the light+polish review flow) or drop the payload/choice bindings — `ConsentStore.resolve(expected_text:, allow_raw:)` must reject reusing a token for a different payload AND reject flipping the raw/redacted choice locked by the first use, both checked BEFORE consuming a use.
...
- Send an Ollama `/api/chat` request without `think` — Gemma 4 reasons by default and polish latency goes from ~0.3 s to ~6 s; `OllamaBackend.polish` sets `think: ollama_think_enabled()`.
- Route rules-only (`activePolishProvider == "rules"`) dictation cleanup through the backend — `DictationWorkflowCoordinator` serves it in-app (`rules · in-app`), and the test pins that no `/v1/cleanup` request is made.
- Reuse the polish memory-guard threshold for smart actions — `_POLISH_PRESSURE_SKIP_LEVEL` (2) vs `_SMART_ACTION_PRESSURE_SKIP_LEVEL` (4) are deliberately different; lowering the smart-action tier re-disables the cockpit on 16 GB machines.
